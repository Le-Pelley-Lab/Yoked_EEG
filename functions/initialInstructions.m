...
if exptSession == 2
    instructStr1 = 'Each trial will start with a cross on the screen. This is to warn you that the trial is about to start, you should keep your eyes fixed on the cross. Then a set of shapes will appear; an example is shown below.';
...
if exptSession == 1
    ...
    im1 = imread('exampleSession2.jpg', 'jpg');
    im2 = imread('exampleSession2-T.jpg', 'jpg');
end
...
exImageRectCentre = [scr_centre(1) - x/2    scr_centre(2)   scr_centre(1) + x/2     scr_centre(2) + y];
...
elseif exptSession == 2
    if instrTrial < 2
        Screen('PutImage', MainWindow, im1, exImageRectCentre);
    elseif instrTrial == 2 || instrTrial == 3 
         Screen('PutImage', MainWindow, im2, exImageRectCentre); 
    end
end
